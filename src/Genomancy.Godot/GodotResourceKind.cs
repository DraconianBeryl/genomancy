namespace Genomancy.Godot;

public static class GodotResourceKind
{
    public const string GenomeVersion = "genomancy.genomeVersion";

    public const string PopulationTemplate = "genomancy.populationTemplate";

    public const string PopulationTemplateGroup = "genomancy.populationTemplateGroup";

    public const string MosaicGenome = "genomancy.mosaicGenome";

    public const string ResourceTests = "genomancy.resourceTests";

    public const string ResourceTestBatchRuns = "genomancy.resourceTestBatchRuns";

    public const string ResourceTestResult = "genomancy.resourceTestResult";

    public const string ResourceTestBatchRunResult = "genomancy.resourceTestBatchRunResult";

    public const string ResourceTestResultManifest = "genomancy.resourceTestResultManifest";
}
