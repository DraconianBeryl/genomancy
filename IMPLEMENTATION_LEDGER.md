...
| Current implementation slice | Slice 43 - resource-test CLI manifest summary repair (verified); later hardening/release work is next |
...
| 2026-06-15 | Expand scope to Slice 39 JSON-file resource-test batch execution workflow. | Project request to implement the next ambitious slice and allow substantial scope expansion | Adds optional storage-backed execution of serialized JSON batch plans with individual result writes, aggregate batch-result writes, and manifest append/upsert updates, without defining CLI commands, repository discovery, retention, locking, or project layout. | Accepted |
...
| 2026-06-15 | Expand scope to Slice 43 resource-test CLI manifest summary repair. | Project request to implement the next ambitious slice and target roughly 600 changed lines | Adds a CLI command that rewrites selected manifest summaries from referenced result files while preserving manifest metadata, with dry-run and filter support, without adding manifest path rewrites, retention cleanup, repository discovery, glob inputs, or machine-readable CLI output. | Accepted |
...
### Slice 35 - Godot bridge for batch-run results

**Status:** Verified on 2026-06-15 for the refined Slice 35 acceptance criteria. GodotSharp resources, editor plugins, binary Godot import/export, persistence policy, result dereferencing, CLI output, and project layout remain **In progress**.

**Objective:** Make aggregate resource-test batch-run results available to package-free Godot-facing callers using the same document bridge pattern already used for single run results, manifests, and batch plans.

**Deliverables**

- Add a Godot-facing resource kind for aggregate resource-test batch-run results.
- Add package-free Godot bridge export/import methods for `ResourceTestBatchRunResult`.
- Preserve canonical Slice 34 batch-result JSON through the Godot document payload.
- Derive Godot document tags from manifest-entry tags and contained case tags.
- Derive system-definition-version metadata from embedded reproducibility packets when present.
- Keep the bridge package-free and avoid adding GodotSharp resources, editor tooling, result dereferencing, or persistence policy.

**Acceptance criteria**

- Godot batch-result document round trips preserve canonical batch-result JSON.
- Godot batch-result documents expose deterministic kind, metadata, and de-duplicated sorted tags.
- Imported Godot batch-result documents reconstruct aggregate status, run records, and generated manifest entries.
- Kind mismatches return adapter diagnostics through the existing import-result surface.
- Core remains independent of Godot.

**Tests**

- Godot batch-result document round trip test.
- Godot batch-result metadata/tag ordering test.
- Godot package lookup and kind-mismatch diagnostic test.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `GodotResourceKind.ResourceTestBatchRunResult`.
- `GodotResourceBridge.ExportResourceTestBatchRunResult`.
- `GodotResourceBridge.ImportResourceTestBatchRunResult`.
- Package-free implementation tests for canonical JSON preservation, metadata/tags, package lookup, and kind mismatch behavior.

**Implementation simplification choices**

- Godot batch-result documents use canonical JSON payloads only; binary Godot resources remain deferred.
- System-definition-version metadata is derived only from embedded reproducibility packets, matching the existing single-run result bridge behavior.
- The adapter does not dereference manifest paths or write result files.

**Not yet implemented**

- GodotSharp `Resource` subclasses, editor plugins, Godot addon layout, `.tres`/`.res` export, binary Godot import/export, manifest/result/batch-plan file dereferencing, persistence policy, CLI output, or Godot engine-version matrices.

**Requirements advanced:** REQ-RTEST, REQ-SERIAL, REQ-GODOT, REQ-VALIDATE.

### Slice 36 - Batch-run summary rollups

**Status:** Verified on 2026-06-15 for the refined Slice 36 acceptance criteria. Serialized summary resources, duration metrics, CLI status output, retention policy, and broader reporting formats remain **In progress**.

**Objective:** Add a reusable derived summary surface for aggregate batch-run results so reports and future tool surfaces can consume one deterministic rollup instead of reimplementing run/case/diagnostic counting.

**Deliverables**

- Add `ResourceTestBatchRunSummary` with aggregate status, run counts, case counts, diagnostic severity counts, and reproducibility-packet counts.
- Derive batch summaries from already-materialized `ResourceTestBatchRunResult` values.
- Treat an empty batch result deterministically as passed with zero counts, matching existing result status behavior.
- Update the batch-run text report formatter to use the shared summary and expose aggregate run/case/diagnostic/packet counts.
- Keep summaries derived only; do not create separate persisted summary resources or CLI output policy.

**Acceptance criteria**

- Summary rollups count total/passed/failed runs.
- Summary rollups count total/passed/failed cases across all runs.
- Summary rollups count diagnostics by severity and reproducibility packets across all runs.
- Empty batch summaries are deterministic and zero-count.
- Batch text reports include the aggregate summary fields while retaining deterministic run and manifest details.

**Tests**

- Batch summary rollup test for mixed passed/failed runs.
- Empty batch summary test.
- Batch text report aggregate header test.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `ResourceTestBatchRunSummary`.
- `ResourceTestBatchRunTextReportFormatter` aggregate summary header.
- Package-free implementation tests for summary counts and report output.

**Implementation simplification choices**

- Batch summaries are derived from manifest-entry run summaries and current run status only; they are not serialized independently.
- Duration, elapsed-time, output-path, retention, and CLI status fields remain deferred.
...
### Slice 37 - Resource-test manifest merge/update utilities

**Status:** Verified on 2026-06-15 for the refined Slice 37 acceptance criteria. Filesystem writes, retention policy, CLI commands, project layout, manifest history/conflict records, and result dereferencing remain **In progress**.

**Objective:** Add deterministic in-memory utilities for combining resource-test result manifests so future storage, CLI, and editor surfaces can update indexes without reimplementing duplicate and replacement behavior.

**Deliverables**

- Add `ResourceTestResultManifestMerger`.
- Support strict append-style manifest merging from an existing manifest plus incoming manifest entries.
- Support merge inputs from complete manifests and aggregate batch-run results.
- Support upsert-style replacement by stable `runId` for rerun/update workflows.
- Preserve existing `ResourceTestResultManifest` ordering, duplicate validation, path trimming, tag sorting, and UTC timestamp normalization.
- Keep the utility storage-agnostic; do not write manifests or results to files.

**Acceptance criteria**

- Strict merge appends new entries and returns deterministic `runId` ordering.
- Strict merge rejects duplicate incoming run IDs.
- Strict merge rejects replacement of existing run IDs through the existing manifest duplicate validation path.
- Upsert replaces existing entries by `runId` and appends new batch-result manifest entries.
- Upsert keeps the last incoming entry for repeated incoming run IDs.
- Entry metadata, derived summaries, labels, tags, paths, and completed timestamps remain normalized after merge/update.

**Tests**

- Manifest merge append, deterministic ordering, metadata preservation, duplicate incoming rejection, and existing-run replacement rejection.
- Batch-result upsert replacement, append behavior, and last-incoming-wins behavior.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `ResourceTestResultManifestMerger.Merge`.
- `ResourceTestResultManifestMerger.Upsert`.
- Overloads for manifests, aggregate batch-run results, and raw manifest entries.
- Package-free implementation tests for strict append and rerun/update workflows.

**Implementation simplification choices**

- Merge/update operates only on already-materialized in-memory values.
- Upsert conflict policy is simple last-incoming-wins by stable `runId`; no conflict diagnostics, audit trail, or source provenance is recorded.
- Strict merge delegates existing-run replacement rejection to the manifest constructor's duplicate-run invariant.
...
### Slice 39 - JSON-file resource-test batch execution workflow

**Status:** Verified on 2026-06-15 for the refined Slice 39 acceptance criteria. Retention policy, project layout, repository discovery, cross-process locking, manifest history/conflict records, result dereferencing, CLI packaging, and parallel execution remain **In progress**.

**Objective:** Compose serialized batch plans, resource-test execution, individual result persistence, aggregate result persistence, and manifest update into one optional JSON-file storage workflow without moving filesystem policy into core.

**Deliverables**

- Add `ResourceTestBatchRunJsonFileWorkflow` in `Genomancy.Storage.JsonFile`.
- Load a serialized resource-test batch plan from a caller-selected JSON file.
- Execute the loaded plan through the core batch runner.
- Write each run's `ResourceTestRunResult` to its plan-specified `resultPath`.
- Resolve relative run result paths against a caller-supplied run-result root, or the batch-plan directory when no root is supplied.
- Write the aggregate `ResourceTestBatchRunResult` to a caller-selected JSON file.
- Optionally update a caller-selected manifest through strict append or upsert/rerun semantics.
- Return an execution result object containing paths, the aggregate batch result, written run-result paths, and the updated manifest when applicable.

**Acceptance criteria**

- A stored batch plan can be loaded and executed without core taking a filesystem dependency.
- Per-run result files are written through the existing typed resource-test result store.
- Nested relative run result paths create required directories under the selected run-result root.
- The aggregate batch-result file reloads canonically through the existing typed batch-result store.
- Upsert manifest execution creates or updates the selected manifest without duplicating rerun entries.
- Strict append manifest execution rejects duplicate run ids after an upserted run exists.

**Tests**

- JSON-file workflow executes a persisted batch plan containing passing and failing runs.
- Individual passing and failing run-result files are written and reload with expected statuses.
- Aggregate batch-result file reloads canonically.
- Manifest upsert creates two deterministic entries and a repeated workflow run keeps two entries.
- Strict append duplicate rejection is exercised after upserted entries exist.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `ResourceTestBatchRunJsonFileWorkflow`.
- `ResourceTestBatchRunJsonFileExecutionResult`.
- Storage-backed batch plan load, batch execution, individual run-result writes, aggregate batch-result write, and optional manifest append/upsert.
- Package-free implementation test for full stored batch execution and persistence behavior.
- Architecture note for the batch execution workflow boundary.

**Implementation simplification choices**

- Run result paths remain the manifest/result paths declared by the plan; the workflow resolves those strings for file writes but does not rewrite manifest entries.
- The workflow is sequential and uses the existing atomic single-file store operations. It does not define cross-process locking, compare-and-swap, or transactional rollback across multiple files.
- Output locations are explicit caller inputs; no repository discovery, default project layout, or retention policy is inferred.

**Not yet implemented**

- Repository/project layout, automatic plan discovery, retention/naming policy, cross-process locking, multi-file transaction rollback, result dereferencing from manifests, merge conflict reports, historical manifest snapshots, CLI packaging/completion, parallel batch execution, or editor/Godot execution actions.

**Requirements advanced:** REQ-RTEST, REQ-VALIDATE, REQ-STORAGE.

### Slice 40 - Resource-test batch CLI host

**Status:** Verified on 2026-06-15 for the refined Slice 40 acceptance criteria. Repository discovery, default project layout, retention policy, packaged tool distribution, shell completion, result dereferencing, richer command families, and parallel execution remain **In progress**.

**Objective:** Provide a package-free optional command-line host that composes the existing JSON-file batch execution workflow into a usable command while preserving the core/storage dependency boundaries.

**Deliverables**

- Add `Genomancy.Cli` as an optional executable project.
- Implement `genomancy batch run` over serialized JSON batch-run plans.
- Require explicit `--plan` and `--batch-result` paths.
- Support optional `--manifest`, `--manifest-mode upsert|append`, and `--run-result-root`.
- Support deterministic human-readable batch report output to `--report` and optional `--stdout-report`.
- Return stable exit codes for success, test failures, usage errors, and execution errors.
- Keep the CLI reusable from tests through an injectable output/error writer entry point.
- Include the CLI in solution build and implementation verification.

**Acceptance criteria**

- A passing stored batch plan executes from CLI arguments and writes individual run results, aggregate batch result, manifest, and text report files.
- CLI stdout includes deterministic status, paths, written result count, and aggregate summary.
- `--stdout-report` prints the existing deterministic batch text report.
- A failing stored batch plan still writes outputs and returns the test-failure exit code.
- Missing/invalid arguments return the usage-error exit code and render usage help.
- Strict manifest append conflicts return the execution-error exit code.
- The CLI project does not add Godot, test-framework, or third-party command-line dependencies.

**Tests**

- CLI execution of a passing serialized batch plan with manifest, run-result root, report file, and stdout report.
- CLI execution of a failing serialized batch plan with test-failure exit code and written output files.
- CLI usage-error handling for a missing required option value.
- CLI strict append duplicate handling as execution error.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `Genomancy.Cli` executable project.
- `GenomancyCli` reusable command runner with injectable output/error writers.
- `GenomancyCliExitCode` stable exit-code enum.
- `batch run` command that delegates to `ResourceTestBatchRunJsonFileWorkflow`.
- Deterministic execution summary and optional report file/stdout rendering.
- Package-free implementation tests for CLI success, failed tests, usage errors, and append conflicts.
- README and architecture notes for the optional CLI boundary.

**Implementation simplification choices**

- Command parsing is intentionally local and minimal; no third-party command-line parser is added.
- The CLI exposes only serialized JSON resource-test batch execution. It does not create plans, inspect manifests, dereference stored results, or run arbitrary resource packs.
- All output locations are explicit arguments or the existing workflow default beside the plan; no repository discovery or default project layout is inferred.
- Exit codes distinguish usage, execution, and resource-test failure, but no machine-readable CLI event stream is defined yet.

**Not yet implemented**

- Packaged global/local .NET tool metadata, shell completion, command aliases, environment-variable defaults, repository discovery, default output layout, retention/naming policy, result dereferencing/list/show commands, standalone manifest commands, plan-generation commands, resource-pack discovery, parallel execution, cross-process locking, multi-file rollback, CI annotations, or editor/Godot command invocation.

**Requirements advanced:** REQ-RTEST, REQ-VALIDATE, REQ-STORAGE.

### Slice 41 - Resource-test CLI inspection and manifest update commands

**Status:** Verified on 2026-06-15 for the refined Slice 41 acceptance criteria. Resource-pack discovery, plan generation, manifest result dereferencing/list/show by run ID, machine-readable event streams, packaged tool distribution, shell completion, and parallel execution remain **In progress**.

**Objective:** Expand the optional CLI from batch execution into a practical artifact inspection/update tool over the JSON resources already produced by earlier slices, while continuing to delegate domain behavior to core and persistence behavior to the JSON-file storage module.

**Deliverables**

- Add `genomancy result show --result <path>` for deterministic text rendering of one stored `ResourceTestRunResult`.
- Add `genomancy batch show --batch-result <path>` for deterministic text rendering of one stored aggregate `ResourceTestBatchRunResult`.
- Add `genomancy manifest show --manifest <path>` for deterministic manifest summaries and entry listings.
- Support manifest show filters for `--status passed|failed` and `--tag <tag>`.
- Support manifest show `--resolve-root <path>` to display resolved paths for relative manifest entries without changing stored paths.
- Add `genomancy manifest update --manifest <path> --from-batch-result <path>` to append/upsert entries from stored aggregate batch results.
- Support optional `--report <path>` output for show/update commands.
- Preserve stable exit-code behavior across show/update commands.

**Acceptance criteria**

- Stored individual run results render through the CLI and return the resource-test failure exit code when the stored result failed.
- Stored aggregate batch results render through the CLI and return the resource-test failure exit code when any contained run failed.
- Manifest show prints total and selected entry counts, deterministic entry ordering, labels, tags, timestamps, diagnostics counts, packet counts, and optional resolved paths.
- Manifest show filters by status and tag without mutating the manifest file.
- Manifest update creates or updates a caller-selected manifest from a stored aggregate batch result using upsert semantics by default.
- Manifest update strict append rejects duplicate run IDs with an execution-error exit code.
- Invalid show/update arguments still return the usage-error exit code.

**Tests**

- CLI stored result show with deterministic report file and failed-result exit code.
- CLI stored batch-result show with deterministic report file and failed-batch exit code.
- CLI manifest show with failed-status/tag filtering, resolved paths, and report output.
- CLI manifest update from stored batch result with upsert/rerun behavior and report output.
- CLI manifest update strict append duplicate rejection.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `batch show`, `result show`, `manifest show`, and `manifest update` commands in `Genomancy.Cli`.
- Deterministic CLI manifest text formatter with selected-count, status/tag filters, labels, timestamps, tags, summaries, and optional resolved paths.
- Manifest update command composed over `ResourceTestResultManifestJsonFileUpdater` and stored aggregate batch results.
- Optional report-file output for inspection/update commands.
- Package-free implementation tests for stored result reports, stored batch reports, manifest filtering/resolution, and manifest update behavior.
- README and architecture notes for the expanded CLI boundary.

**Implementation simplification choices**

- Manifest formatting is currently CLI-local because it is a host/reporting concern and does not need a new core serialization contract yet.
- Manifest show displays resolved paths but does not load or dereference the result files they point at.
- Manifest update accepts stored aggregate batch-result JSON only; it does not accept raw entries, individual result files, resource packs, or globbed directories.
...
- Result dereferencing from manifests, manifest `list`/`show-run` commands, individual-result-to-manifest entry creation, plan generation, resource-pack discovery, directory/glob inputs, machine-readable JSON CLI output/events, CI annotations, packaged global/local .NET tool metadata, shell completion, environment-variable defaults, default project layout, retention policy, cross-process locking, multi-file rollback, or parallel execution.

**Requirements advanced:** REQ-RTEST, REQ-VALIDATE, REQ-STORAGE.

### Slice 42 - Resource-test CLI manifest result dereferencing and verification

**Status:** Verified on 2026-06-15 for the refined Slice 42 acceptance criteria. Repository discovery, glob/directory inputs, retention policy, machine-readable CLI event output, packaged tool distribution, shell completion, and parallel verification remain **In progress**.

**Objective:** Make stored result manifests actionable from the CLI by loading the result file behind a manifest entry, rendering that result in context, and verifying that manifest summaries match the referenced result artifacts.

**Deliverables**

- Add `genomancy manifest result show --manifest <path> --run-id <id> --result-root <path>`.
- Resolve relative manifest entry result paths against an explicit result root while preserving stored manifest paths.
- Render manifest entry metadata, resolved path, summary-match status, and the linked resource-test result report.
- Reject missing run IDs, missing result files, and summary mismatches with execution-error exit codes.
- Add `--allow-summary-mismatch` for diagnostic rendering of mismatched linked results.
- Add `genomancy manifest verify --manifest <path> --result-root <path>`.
- Verify selected manifest entries by loading referenced result files and comparing all summary fields.
- Support `manifest verify` status/tag filters and optional report output.
- Return execution error for missing or mismatched artifacts, resource-test failure for valid failed results, and success for valid passing selections.

**Acceptance criteria**

- A manifest entry can be dereferenced by stable run ID and rendered with linked result diagnostics.
- Dereferenced result rendering reports whether the stored manifest summary matches the loaded result summary.
- Summary mismatch blocks dereferenced rendering by default and can be explicitly allowed for diagnostics.
- Manifest verification reports selected, verified, missing, mismatched, and failed-result counts.
- Manifest verification detects missing result files.
- Manifest verification detects mismatched result summaries.
- Status filters affect verification selection and exit-code behavior.

**Tests**

- CLI manifest result show by run ID with matched summary, failed-result exit code, and report output.
- CLI manifest verification of a mixed passed/failed manifest with all result files present.
- CLI manifest verification filtered to passing entries returning success.
...
- `manifest result show` command in `Genomancy.Cli`.
- `manifest verify` command in `Genomancy.Cli`.
- CLI-local manifest-result and verification report formatters.
- Summary comparison over status, case counts, diagnostic counts, and reproducibility-packet counts.
- Package-free implementation tests for dereferencing, verification, missing files, summary mismatch rejection, and allowed mismatch rendering.
- README and architecture notes for manifest dereferencing/verification.

**Implementation simplification choices**

- Result dereferencing requires an explicit `--result-root`; no repository discovery, manifest-relative default, or project layout convention is inferred.
- Verification is sequential and uses existing typed JSON-file stores; no parallel traversal or cross-process locking is added.
- Summary comparison is exact over current summary fields; it does not attempt partial compatibility across future summary schema versions.
- Verification reports are deterministic text only; no machine-readable CLI JSON contract is introduced.

**Not yet implemented**

- Manifest-relative/default result-root discovery, directory/glob verification inputs, retention cleanup, manifest repair/rewrite from loaded results, machine-readable CLI output/events, CI annotations, packaged global/local .NET tool metadata, shell completion, environment-variable defaults, cross-process locking, multi-file rollback, or parallel verification.

**Requirements advanced:** REQ-RTEST, REQ-VALIDATE, REQ-STORAGE.

### Slice 43 - Resource-test CLI manifest summary repair

**Status:** Verified on 2026-06-15 for the refined Slice 43 acceptance criteria. Manifest path rewriting, retention cleanup, repository discovery, glob/directory inputs, machine-readable CLI event output, packaged tool distribution, shell completion, cross-process locking, and parallel repair remain **In progress**.

**Objective:** Let the optional CLI repair stale manifest summaries from the authoritative stored result files discovered in Slice 42, while preserving caller-authored manifest identity/path/metadata and refusing partial writes when selected artifacts are missing or unreadable.

**Deliverables**

- Add `genomancy manifest repair --manifest <path> --result-root <path>`.
- Reuse manifest verification result loading and exact summary comparison.
- Rewrite selected manifest entry summaries from loaded result summaries.
- Preserve run ID, result path, completed timestamp, label, and tags.
- Support `--status passed|failed` and `--tag <tag>` filters.
- Support `--dry-run` report-only mode that leaves the manifest file unchanged.
- Support optional `--report <path>` output.
- Refuse to save when any selected result file is missing or unreadable.
- Return resource-test failure when selected repaired/verified result files include failed results.

**Acceptance criteria**

- A stale manifest with result files present can be repaired and then verified cleanly.
- Repair updates failed-result summary status and diagnostic counts from the result file.
- Repair preserves manifest metadata such as label and sorted tags.
- Dry-run repair reports proposed changes without saving.
- Filtered repair affects only selected manifest entries.
- Missing selected result files return an execution-error exit code and leave the manifest unchanged.

**Tests**

- CLI manifest repair rewrites stale summaries from stored result files and preserves metadata.
- Repaired manifest verifies with zero missing and zero mismatched entries.
- CLI dry-run repair reports selected repairs and leaves manifest JSON unchanged.
- CLI repair reports missing selected result files without saving the manifest.
- Full build/test verification through `scripts/verify.sh`.

**Implemented**

- `manifest repair` command in `Genomancy.Cli`.
- CLI-local manifest repair report formatter.
- Summary repair preserving manifest entry identity/path/timestamp/label/tags.
- Dry-run and status/tag filter support.
- Missing/unreadable result artifact blocking before save.
- Package-free implementation tests for write repair, dry-run preservation, and missing-file no-save behavior.
- README and architecture notes for manifest repair.

**Implementation simplification choices**

- Repair updates summaries only; it does not rewrite result paths, labels, timestamps, tags, or ordering.
- Repair requires explicit `--result-root`; no manifest-relative default or repository layout convention is inferred.
- Repair is sequential and single-manifest only; no glob inputs, parallel repair, cross-process locking, or multi-file rollback are added.
- Repair reports are deterministic text only; no machine-readable CLI JSON/event contract is introduced.

**Not yet implemented**

- Manifest path normalization/rewriting, retention cleanup, manifest compaction, bulk directory repair, repository discovery, default result-root inference, machine-readable CLI output/events, CI annotations, packaged global/local .NET tool metadata, shell completion, environment-variable defaults, cross-process locking, multi-file rollback, or parallel repair.
...
- Slice 35 Godot bridge for batch-run results:
  - package-free Godot resource kind and bridge methods for aggregate batch-run results
  - deterministic document metadata and tag derivation
  - package lookup, kind mismatch, and canonical JSON preservation coverage
- Slice 36 batch-run summary rollups:
  - reusable derived aggregate batch-run summary
  - total/passed/failed run and case counts
  - diagnostic severity and reproducibility-packet rollups
  - batch text report summary header backed by the shared summary
- Slice 37 resource-test manifest merge/update utilities:
  - deterministic strict append merge for result manifests
  - deterministic upsert replacement by stable run id
...
- Slice 39 JSON-file resource-test batch execution workflow:
  - optional storage-backed batch plan execution workflow
  - persisted individual resource-test run results
  - persisted aggregate batch-run results
  - manifest append/upsert integration for executed batch results
  - explicit result-root resolution for relative per-run result paths
- Slice 40 resource-test batch CLI host:
  - optional `Genomancy.Cli` executable project
  - `batch run` command for serialized JSON batch-run plans
  - explicit output paths for aggregate batch results, optional manifests, per-run result roots, and text reports
  - stable exit codes for success, resource-test failure, usage error, and execution error
  - deterministic stdout summary and optional batch text report emission
- Slice 41 resource-test CLI inspection and manifest update commands:
  - `result show` command for stored individual resource-test result reports
  - `batch show` command for stored aggregate batch-result reports
  - `manifest show` command with status/tag filters and optional resolved paths
  - `manifest update` command that appends/upserts entries from stored aggregate batch results
  - optional report-file output for show/update commands
- Slice 42 resource-test CLI manifest result dereferencing and verification:
  - `manifest result show` command for run-ID-based manifest entry dereferencing
  - linked result report rendering with manifest metadata and summary-match status
...
- Slice 43 resource-test CLI manifest summary repair:
  - `manifest repair` command for selected manifest entries
  - summary rewrites from referenced result files while preserving manifest metadata
  - dry-run repair reporting without saving
  - status/tag-filtered repair selection
  - missing/unreadable selected result artifact blocking before save
...
- Resource-pack loading, serialized operation/assertion registries beyond validation/freeze/assertions and the Slice 17 population-template frequency assertion, snapshots, fuzz/matrix execution, broader statistical assertions, validation reachability/policy coverage assertions, runtime-safe subset handling, CLI commands beyond batch execution/inspection/manifest update/manifest dereferencing/manifest summary repair, CLI packaging/completion, editor integration, manifest retention/naming policy, project result-file layout, compact/final batch-plan and batch-result binary schemas, and cross-process storage concurrency controls.
...
- Slice 39 executes one caller-selected batch plan at a time and writes caller-routed JSON outputs; it does not define repository discovery, default output layout, retention policy, CLI packaging, cross-process locking, multi-file rollback, or parallel execution.
...
- Slice 43 repairs only manifest summaries from referenced result files and preserves manifest metadata; it does not rewrite paths, infer result roots, repair globbed directories, clean old results, emit machine-readable CLI JSON, or parallelize repair.
...
- Slice 35 package-free implementation tests in `tests/Genomancy.Tests`:
  - Godot adapter aggregate batch-result document round trip
  - Godot batch-result metadata and tag ordering
  - canonical batch-result JSON preservation through Godot document payloads
  - Godot package lookup and kind-mismatch diagnostics for batch-result documents
- Slice 36 package-free implementation tests in `tests/Genomancy.Tests`:
  - aggregate batch-run summary rollup for mixed passed/failed runs
  - empty batch-run summary zero-count behavior
  - batch text report aggregate run, case, diagnostic, and packet summary rendering
- Slice 37 package-free implementation tests in `tests/Genomancy.Tests`:
  - strict manifest merge append, deterministic ordering, and metadata preservation
  - duplicate incoming run-ID rejection
...
- Slice 39 package-free implementation tests in `tests/Genomancy.Tests`:
  - JSON-file workflow execution of a stored batch plan
  - individual passing and failing run-result file persistence
  - nested relative run-result path directory creation under an explicit output root
  - aggregate batch-result canonical reload
  - manifest upsert rerun behavior and strict append duplicate rejection
- Slice 40 package-free implementation tests in `tests/Genomancy.Tests`:
  - CLI execution of a passing stored batch plan with run-result, aggregate, manifest, and report writes
  - deterministic CLI stdout summary and optional stdout batch report
  - CLI execution of a failing stored batch plan with test-failure exit code
  - CLI usage error for missing option value
  - CLI strict manifest append conflict as execution error
- Slice 41 package-free implementation tests in `tests/Genomancy.Tests`:
  - CLI stored individual result report rendering with report-file output
  - CLI stored aggregate batch-result report rendering with report-file output
  - CLI manifest show status/tag filtering, selected-count rendering, resolved paths, and report-file output
  - CLI manifest update from stored aggregate batch results with upsert/rerun behavior
  - CLI manifest update strict append duplicate rejection
- Slice 42 package-free implementation tests in `tests/Genomancy.Tests`:
  - CLI manifest result show by run ID with matched summary and report output
  - CLI manifest verification with all result files present
  - CLI manifest verification filtered to passing entries
  - CLI manifest verification missing-file detection
  - CLI manifest result show summary mismatch rejection and explicit mismatch rendering
- Slice 43 package-free implementation tests in `tests/Genomancy.Tests`:
  - CLI manifest repair rewrites stale summaries from stored result files
  - repair preserves labels and tags
  - repaired manifests verify with zero missing and zero mismatched entries
  - CLI dry-run repair leaves manifest JSON unchanged
  - CLI repair missing-file rejection leaves manifest JSON unchanged
...
- Slice 39 acceptance criteria are verified by `scripts/verify.sh`.
