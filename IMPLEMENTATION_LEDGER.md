...
| Current implementation slice | Slice 17 - Storage manifests and write-entry metadata |
...
| 2026-06-16 | Continue deferring direct resource-test framework work and use Slice 17 for storage manifests and write-entry metadata. | Project request | Adds storage index/provenance infrastructure useful for later reproducibility and tooling without adding resource-test definitions, operations, assertions, or runners. Resource testing shifts to Slice 18+. | Accepted |
...
| REQ-RTEST | First-class immutable-input resource test definitions, fixtures, operations, assertions, diagnostics, and runners. | 26, 27 | Planned | 18+ | Self-tests + integration |
...
| REQ-VALIDATE | Resource graph, reachability, policy coverage, invariants, negative cases, and required baseline content tests. | 26.19-26.22, 26.39 | In progress | 1, 18+ | Validation + resource tests |
| REQ-SERIAL | Stable JSON and binary formats at multiple granularities, including versions, variants, templates, tests, and failure packets. | 31.1-31.3 | In progress | 2 onward; finalized 19 | Round-trip + compatibility |
| REQ-STORAGE | Core has no permanent storage; optional JSON-file, binary-file, and SQLite modules depend on core. | 31.4-31.7 | In progress | 14-17, 19 | Integration tests |
| REQ-GODOT | Optional Godot adapter consumes core APIs without redefining genetics behavior. | Project scope injection | Planned | 20 | Build + adapter tests |
...
- Resource-authored generated complement policies, generated body structures beyond group state, runtime variant persistence in genome versions, final compact variant binary layout, variant migration, advanced expression policy language, trace/non-ploidal expression effects, and resource-test coverage.
...
**Status:** Verified on 2026-06-06 for the refined Slice 10 acceptance criteria. Broader requirement families remain **In progress** where later slices add template groups, biased inheritance/mutation hooks, statistical tolerances, and final compact binary layouts.
...
- Biased inheritance/mutation hooks, statistical tolerance reports, final compact binary template layout, storage modules, and resource-test coverage.
...
- External template-group reference resolution, resource-authored template-group validation, pair-specific blend matrices, structure-level statistical reports, biased inheritance/mutation hooks, final compact binary template-group layout, and resource-test coverage.

**Requirements advanced:** REQ-TGROUP, REQ-TEMPLATE, REQ-RANDOM.

### Slice 12 - Template-group serialization and generation manifests

**Status:** Verified on 2026-06-16 for the refined Slice 12 acceptance criteria. Broader requirement families remain **In progress** where later slices add final compatibility contracts, statistical tolerance reports, resource tests, and storage modules.
...
- External template/template-group registries, migration hooks, compatibility matrices, statistical tolerance reports, and storage modules.

**Requirements advanced:** REQ-TGROUP, REQ-TEMPLATE, REQ-RANDOM, REQ-SERIAL, REQ-STORAGE.

### Slice 13 - JSON-backed binary serialization expansion

**Status:** Verified on 2026-06-16 for the refined Slice 13 acceptance criteria. Broader requirement families remain **In progress** where later slices finalize compact binary layouts, compatibility matrices, storage modules, and resource tests.

**Objective:** Expand preliminary binary serialization coverage for already-serializable core resources without implementing direct resource-test framework behavior.

**Deliverables**

- Add a shared internal JSON-backed binary envelope helper for preliminary binary payloads.
- Keep the existing genome binary codec behavior while centralizing common header, length, truncation, and payload extraction logic.
- Add binary stream and buffer APIs for population template versions.
- Add binary stream and buffer APIs for population template-group versions.
- Add binary stream and buffer APIs for generated-population manifests.
- Add binary stream and buffer APIs for runtime body-plan variants.
- Preserve system-definition version compatibility checks by delegating decoded JSON payloads through existing JSON codecs.

**Acceptance criteria**

- Binary round trips preserve population template profiles.
- Binary round trips preserve nested population template-group profiles and canonical JSON equivalence.
- Binary round trips preserve generation manifest sample seeds, IDs, group paths, and blend provenance.
- Binary round trips preserve runtime body-plan variant state.
- Truncated binary payloads fail with `GenomeSerializationException`.
- Binary deserialization rejects mismatched system-definition versions.
...
- Population template binary round-trip, stream round-trip, truncation rejection, and version-rejection test.
- Template-group binary round-trip, canonical JSON equivalence, stream round-trip, truncation rejection, and version-rejection test.
- Template-population manifest binary round-trip, stream round-trip, provenance preservation, truncation rejection, and version-rejection test.
- Runtime body-plan variant binary round-trip, stream round-trip, truncation rejection, and version-rejection test.

**Implemented**

- `JsonBackedBinaryEnvelopeCodec` shared internal helper.
- `PopulationTemplateBinaryCodec`.
- `PopulationTemplateGroupBinaryCodec`.
- `TemplatePopulationManifestBinaryCodec`.
- `RuntimeBodyPlanVariantBinaryCodec`.
- `GenomeBinaryCodec` now uses the shared helper while preserving its existing public API.

**Implementation simplification choices**

- Binary codecs are preliminary JSON-backed envelopes with per-resource magic headers, matching the earlier genome binary simplification.
- The slice does not define final compact binary field layouts, migration tables, or cross-version compatibility matrices.
- Binary payload validation delegates semantic version compatibility to the existing JSON codecs.

**Not yet implemented**

- Direct resource-test framework work: test resources, fixtures, operations, assertions, runners, failure packets, tags, severity, snapshots, fuzz/matrix execution, and runtime-safe resource-test subsets.
- Final compact binary layouts, binary compatibility matrices, storage modules, serialized resource-test/failure-packet codecs, and Godot adapter serialization.

**Requirements advanced:** REQ-SERIAL, REQ-TEMPLATE, REQ-TGROUP, REQ-VARIANT, REQ-RANDOM, REQ-STORAGE.

### Slice 14 - Optional JSON file storage module

**Status:** Verified on 2026-06-16 for the refined Slice 14 acceptance criteria. Broader requirement families remain **In progress** where later slices add SQLite storage, final compatibility contracts, resource tests, and Godot adapter storage workflows. Binary-file storage was later added in Slice 15.

**Objective:** Add the first optional non-core storage module for JSON file persistence while keeping `Genomancy.Core` free of filesystem ownership.

**Deliverables**

- Add `Genomancy.Storage.Json` as a separate project that depends on `Genomancy.Core`.
- Keep `Genomancy.Core` independent from the storage project and filesystem storage behavior.
- Implement a root-scoped JSON file store with relative-path resolution.
- Reject absolute paths, parent-directory traversal, empty path segments, and paths escaping the configured root.
- Support optional directory creation and overwrite control.
- Use same-directory temporary files followed by move into place for resource writes.
- Persist and load genome versions, population template versions, population template-group versions, generated-population manifests, and runtime body-plan variants using existing JSON codecs.
- Preserve existing system-definition version checks during file reads.

**Acceptance criteria**

- The JSON storage project builds as an optional module depending on core.
- Core does not reference `Genomancy.Storage.Json`.
- Stored resources round trip through JSON files and preserve existing codec semantics.
- Incompatible system-definition versions are rejected on load.
- Unsafe paths and overwrite conflicts are rejected before writing.
- Direct resource-test framework definitions, operations, assertions, diagnostics, and runners remain unimplemented in this slice.

**Tests**

- Storage dependency-boundary test proving storage references core and core does not reference storage.
- JSON file storage round-trip test for genome versions, population templates, template groups, generated-population manifests, and runtime variants.
- Version-rejection test through the storage read path.
- Path traversal, absolute path, empty segment, missing file, and overwrite-conflict rejection test.

**Implemented**

- `src/Genomancy.Storage.Json/Genomancy.Storage.Json.csproj`.
- `JsonFileStoreOptions`, `JsonFileStoreException`, `JsonFileWriteResult`, and `JsonFileStore`.
- Solution and test project references for the optional storage module.

**Implementation simplification choices**

- Storage is root-scoped and caller-directed; the module does not prescribe a mandatory repository layout, naming convention, manifest index, or save-game model.
- Only JSON file storage is implemented. Binary-file and SQLite modules remain deferred.
- The write path reports full path and byte count, but does not yet expose checksums, content hashes, migration metadata, or batch transaction groups.
- JSON storage delegates all schema and version semantics to existing core JSON codecs.

**Not yet implemented**

- Direct resource-test framework work: test resources, fixtures, operations, assertions, runners, failure packets, tags, severity, snapshots, fuzz/matrix execution, and runtime-safe resource-test subsets.
- Binary-file storage, SQLite storage, storage manifests/indexes, checksums, migration workflows, batch transactions, storage-level compatibility reports, and Godot adapter storage.

**Requirements advanced:** REQ-STORAGE, REQ-SERIAL, REQ-ID, REQ-GENOME, REQ-TEMPLATE, REQ-TGROUP, REQ-VARIANT.

### Slice 15 - Optional binary file storage module

**Status:** Verified on 2026-06-16 for the refined Slice 15 acceptance criteria. Broader requirement families remain **In progress** where later slices add SQLite storage, final compatibility contracts, resource tests, and Godot adapter storage workflows.

**Objective:** Add an optional non-core storage module for preliminary binary resource persistence while keeping `Genomancy.Core` free of filesystem ownership.

**Deliverables**

- Add `Genomancy.Storage.Binary` as a separate project that depends on `Genomancy.Core`.
- Keep `Genomancy.Core` independent from the binary storage project and filesystem storage behavior.
- Implement a root-scoped binary file store with relative-path resolution.
- Reject absolute paths, parent-directory traversal, empty path segments, and paths escaping the configured root.
- Support optional directory creation and overwrite control.
- Use same-directory temporary files followed by move into place for resource writes.
- Persist and load genome versions, population template versions, population template-group versions, generated-population manifests, and runtime body-plan variants using existing preliminary binary codecs.
- Preserve existing system-definition version checks and binary truncation/header checks during file reads.

**Acceptance criteria**

- The binary storage project builds as an optional module depending on core.
- Core does not reference `Genomancy.Storage.Binary`.
- Stored resources round trip through binary files and preserve existing codec semantics.
- Incompatible system-definition versions are rejected on load.
- Truncated binary files are rejected on load.
- Unsafe paths and overwrite conflicts are rejected before writing.
- Direct resource-test framework definitions, operations, assertions, diagnostics, and runners remain unimplemented in this slice.

**Tests**

- Binary storage dependency-boundary test proving storage references core and core does not reference storage.
- Binary file storage round-trip test for genome versions, population templates, template groups, generated-population manifests, and runtime variants.
- Version-rejection and truncation-rejection tests through the storage read path.
- Path traversal, absolute path, empty segment, missing file, and overwrite-conflict rejection test.

**Implemented**

- `src/Genomancy.Storage.Binary/Genomancy.Storage.Binary.csproj`.
- `BinaryFileStoreOptions`, `BinaryFileStoreException`, `BinaryFileWriteResult`, and `BinaryFileStore`.
- Solution and test project references for the optional binary storage module.

**Implementation simplification choices**

- Storage is root-scoped and caller-directed; the module does not prescribe a mandatory repository layout, naming convention, manifest index, or save-game model.
- Binary storage writes the current preliminary JSON-backed binary codec payloads; final compact binary layouts remain deferred.
- The write path reports full path and byte count, but does not yet expose checksums, content hashes, migration metadata, or batch transaction groups.
- Binary storage delegates all schema, magic-header, length, and version semantics to existing core binary codecs.

**Not yet implemented**

- Direct resource-test framework work: test resources, fixtures, operations, assertions, runners, failure packets, tags, severity, snapshots, fuzz/matrix execution, and runtime-safe resource-test subsets.
- SQLite storage, storage manifests/indexes, checksums, migration workflows, batch transactions, storage-level compatibility reports, final compact binary layouts, and Godot adapter storage.
...
### Slice 17 - Storage manifests and write-entry metadata

**Status:** Verified on 2026-06-16 for the refined Slice 17 acceptance criteria. Broader requirement families remain **In progress** where later slices add SQLite storage, final compatibility contracts, resource tests, and Godot adapter storage workflows.

**Objective:** Add storage manifest/index infrastructure over existing storage writes without implementing direct resource-test framework behavior.

**Deliverables**

- Add storage resource format and kind enums in `Genomancy.Storage.Common`.
- Add `StoredResourceEntry` metadata for resource kind, format, relative path, full path, resource ID, resource version ID, system-definition version, byte count, and SHA-256 hash.
- Add immutable `StorageManifest` containing same-system-version stored resource entries.
- Add deterministic JSON codec for storage manifests through streams, buffers, and text.
- Extend JSON and binary file write results with a `StoredResourceEntry`.
- Populate write-result manifest entries for genome versions, population template versions, template-group versions, generated-population manifests, and runtime body-plan variants.

**Acceptance criteria**

- Mixed JSON/binary storage writes can be collected into a deterministic storage manifest.
- Manifest entries preserve resource kind, format, path, ID/version, byte count, system-definition version, and hash metadata.
- Storage manifest JSON round trips preserve ordering and metadata.
- Storage manifests reject entries from mismatched system-definition versions.
- Malformed storage manifest JSON is rejected with a storage manifest exception.
- Direct resource-test framework definitions, operations, assertions, diagnostics, and runners remain unimplemented in this slice.

**Tests**

- Mixed JSON/binary write metadata manifest test.
- Storage manifest JSON round-trip and canonical text test.
- Manifest entry hash verification against stored file contents.
- Manifest mixed-format metadata coverage for JSON genome entries and binary generated-population manifest entries.
- Mismatched manifest entry system-definition rejection test.
- Malformed manifest JSON rejection test.

**Implemented**

- `StoredResourceFormat`, `StoredResourceKind`, `StoredResourceEntry`, `StorageManifest`, `StorageManifestJsonCodec`, and `StorageManifestException`.
- `JsonFileWriteResult.ManifestEntry` and `BinaryFileWriteResult.ManifestEntry`.
- Manifest-entry population in `JsonFileStore` and `BinaryFileStore`.

**Implementation simplification choices**

- Storage manifests are storage indexes/provenance packets only; they are not resource-test definitions, test suites, assertions, or failure packets.
- Manifest entries use string IDs to keep `Genomancy.Storage.Common` independent from `Genomancy.Core`.
- Runtime body-plan variants currently use the variant ID as both resource ID and resource-version ID because they do not yet have a separate version ID.
- Manifest JSON is deterministic but not a final compatibility matrix or migration format.

**Not yet implemented**

- Direct resource-test framework work: test resources, fixtures, operations, assertions, runners, failure packets, tags, severity, snapshots, fuzz/matrix execution, and runtime-safe resource-test subsets.
- SQLite storage, manifest signing, manifest file stores, migration workflows, batch transactions, storage-level compatibility reports, final compact binary layouts, per-resource structured hashes, and Godot adapter storage.

**Requirements advanced:** REQ-STORAGE, REQ-SERIAL, REQ-ID, REQ-RANDOM.

### Slice 18+ - Resource testing framework
...
### Slice 19 - Serialization hardening and remaining optional storage modules
...
### Slice 20 - Godot adapter and packaging
...
- Slice 13 JSON-backed binary serialization expansion:
  - shared internal JSON-backed binary envelope helper
  - genome binary codec centralization on the shared helper
  - binary codecs for population templates, population template groups, generated-population manifests, and runtime body-plan variants
- Slice 14 optional JSON file storage:
  - `Genomancy.Storage.Json` optional module outside core
  - root-scoped JSON file store with safe relative-path resolution
  - atomic same-directory temporary-file writes with overwrite control
  - JSON file read/write support for genome versions, population templates, template groups, generated-population manifests, and runtime body-plan variants
- Slice 15 optional binary file storage:
  - `Genomancy.Storage.Binary` optional module outside core
...
- Slice 17 storage manifests and write-entry metadata:
  - storage resource kind/format enums and immutable stored-resource entries
  - deterministic JSON codec for storage manifests
  - JSON and binary write results now include manifest entries
  - manifest entries capture resource kind, format, relative/full path, resource ID/version, system-definition version, byte count, and SHA-256 hash
...
- Resource-authored generated complement policies, generated structures beyond group state, variant persistence in genome versions, and final compact binary variant layouts.
...
- Final compact serialization formats, SQLite storage, persistent manifest file-store workflows, structured/per-section checksums, signed indexes, and migration workflows.
...
- Slice 12 keeps template-group serialization by-value and manifest-only for generated population reproducibility; external registries, full generated genome packets, and statistical aggregate reports are deferred.
...
- Slice 17 adds deterministic storage manifests as index/provenance metadata only; manifest signing, manifest file-store workflows, compatibility reports, and resource-test failure packets are deferred.
- Preliminary Slice 2 serialization covers only then-existing models; complete format stabilization is deferred to Slice 19.
...
- Slice 13 package-free implementation tests in `tests/Genomancy.Tests`:
  - runtime body-plan variant binary round trip, stream round trip, truncation rejection, and version rejection
  - population template binary round trip, stream round trip, truncation rejection, and version rejection
  - nested template-group binary round trip, canonical JSON equivalence, stream round trip, truncation rejection, and version rejection
  - generated-population manifest binary round trip, stream round trip, provenance preservation, truncation rejection, and version rejection
- Slice 14 package-free implementation tests in `tests/Genomancy.Tests`:
  - JSON storage dependency-boundary test proving storage depends on core and core does not depend on storage
  - JSON file storage round trip for genome versions, population templates, template groups, generated-population manifests, and runtime variants
  - storage read version-rejection through existing core codecs
...
- Slice 15 package-free implementation tests in `tests/Genomancy.Tests`:
  - binary storage dependency-boundary test proving storage depends on core and core does not depend on storage
...
- Slice 17 package-free implementation tests in `tests/Genomancy.Tests`:
  - mixed JSON/binary storage manifest creation from write-result entries
  - storage manifest JSON round trip and canonical text preservation
  - manifest entry hash verification against stored file contents
  - manifest same-system-version enforcement and malformed JSON rejection
...
|---|---|---|---|
| OPEN-001 | Supported Godot adapter version range beyond the initial local Godot 4.6.2 environment. | Slice 20 | Initial core target is `net9.0`; adapter compatibility remains open until Godot adapter refinement. |
| OPEN-002 | Binary format design and compatibility strategy. | Slice 2 | Use versioned preliminary JSON-backed codecs, then stabilize final compact layouts in Slice 19. |
...
| OPEN-008 | SQLite provider and native-binary implications for Godot export targets. | Slice 19 | Keep provider outside core and evaluate platform support before selection. |
